...
def cl_arguments():
    ...
    parent_parser.add_argument("--cge", help="Create an output with the standard CGE format", action="store_true")
    ...
    inference_parser = subparsers.add_parser("predict", help="Predict using PathogenFinder2 model.", parents=[parent_parser])
    inference_parser.set_defaults(action="Prediction")
    inference_parser.add_argument("-i", "--inputData", help="Input data that will be used for pathogenic capacity prediction")
