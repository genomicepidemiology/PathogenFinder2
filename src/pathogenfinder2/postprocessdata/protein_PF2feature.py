...
class MapProteins:
    ...
    def __init__(self, folder_out:str, db_path:str, diamond_path:str,
                    log_folder:str=None, folder_tmp:str=None, metadata:str=None):
    ...
    def check_metadata(metadata):
        for col in ["Entry", "Entry ID", "GO Term"]:
            if "Entry" not in metadata.columns:
                raise KeyError("""The column 'Entry' does not appear on the metadata file. Check if it comes from Swiss-Prot, """
                                """if the correct columns have been downloaded, and if it has been formated correctly using """
                                """pathogenfinder2 setup_gsea""".format(metadata))
    ...
    def load_attfile(attfile, combine_max=True, top_proteins=20):
        ...
        attdf["protein_id"] = attdf["protein_id"].str.decode("utf-8")
    ...
    def run_gsea(map_df, att_df, threads=1, min_size=15):
        ...
        mapping_sseqid = gsea_df.set_index("qseqid")["sseqid"]
        mapping_descr = gsea_df.set_index("qseqid")["protein_description"]
    ...
    def sort_aln_results(self, att_df: pd.DataFrame, map_df: pd.DataFrame) -> pd.DataFrame:
        ...
        map_results = map_results.astype({
            ...
            "length": "float", "slen": "float", "qstart": "float", "qend": "float", "sstart":"float",
            "send": "float"
        })
    ...
    def map_proteins(self, att_file:str, prot_file:str, gsea:bool=True, top_proteins:int=20, gsea_minsize:int=15):
        fsa_dict = MapProteins.load_fsafile(file_path=prot_file)
...
if __name__ == "__main__":
    mapprot = MapProteins(folder_out="/work3/alff/PathogenFinder2/outtest/testaln/",
                          diamond_path="/work3/alff/PathogenFinder2/bin/diamond",
                          db_path="/work3/alff/PathogenFinder2/data/proteindb/uniprotkb_taxonomy_id_2_AND_reviewed_tr_2025_11_21_diamond",
                          metadata="/work3/alff/PathogenFinder2/data/proteindb/uniprotkb_taxonomy_id_2_AND_reviewed_tr_2025_11_21_formated.tsv")
    mapprot.map_proteins(att_file="outtest/predictionPF2/results_PF2/attentions.npz",
                            prot_file="outtest/predictionPF2/preprocess_PF2/PredictedProteins.faa", gsea=True, top_proteins=20,
                            gsea_minsize=2)
